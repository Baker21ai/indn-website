...
export function EditSponsorForm({ sponsor }: EditSponsorFormProps) {
  const router = useRouter()
  const fileInputRef = useRef<HTMLInputElement>(null)

  const [isSubmitting, setIsSubmitting] = useState(false)
  const [uploadedLogoUrl, setUploadedLogoUrl] = useState<string | null>(sponsor.logoUrl)
  ...
  const [formData, setFormData] = useState({
    ...
    tier: sponsor.tier || '',
    totalAmount: sponsor.totalAmount.toString(),
    location: sponsor.location || '',
    website: sponsor.website || '',
    notes: sponsor.notes || '',
    memberSince: new Date(sponsor.memberSince).toISOString().split('T')[0],
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Failed to update sponsor. Please try again.')
    } finally {
      setIsSubmitting(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <RadioGroup
              ...
              onValueChange={(value) => setFormData({ ...formData, sponsorType: value as 'individual' | 'company' })}
            >
