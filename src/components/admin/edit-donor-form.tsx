...
export function EditDonorForm({ donor }: EditDonorFormProps) {
  const router = useRouter()
  const fileInputRef = useRef<HTMLInputElement>(null)

  const [isSubmitting, setIsSubmitting] = useState(false)
  const [uploadedLogoUrl, setUploadedLogoUrl] = useState<string | null>(donor.logoUrl)
  ...
  const [formData, setFormData] = useState({
    ...
    notes: donor.notes || '',
    memberSince: new Date(donor.memberSince).toISOString().split('T')[0],
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        totalDonated: parseFloat(formData.totalDonated) || 0,
      ...
      router.push('/portal/admin/donors')
      router.refresh()
    } catch (error) {
      console.error('Error updating donor:', error)
      alert(error instanceof Error ? error.message : 'Failed to update donor. Please try again.')
    } finally {
      setIsSubmitting(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <RadioGroup
              ...
              onValueChange={(value) => setFormData({ ...formData, donorType: value as 'individual' | 'company' })}
            >
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={
                formData.donorType === 'company' ? 'Acme Corporation' : 'John Doe'
              }
              required
            />
            <p className="text-xs text-muted-foreground">
              This name will appear on the donor wall
            </p>
          </div>

          {/* Tier and Total Donated */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, totalDonated: e.target.value })}
                placeholder="0.00"
              />
            </div>
          </div>
                ...
                <div className="relative border-2 border-dashed border-gray-300 rounded-lg p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-green-600 font-medium mb-2">Logo uploaded</p>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isSubmitting ? 'Updating Donor...' : 'Update Donor'}
            </Button>
            <Button
              type="button"
              variant="outline"
